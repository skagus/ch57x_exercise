...
void	os_handleAsyncEvt(void);
void	os_applyEvt(uint32 bmNewEvt);


void os_SetNextTask(void)
{
...
__attribute__((naked)) void os_Switch()
{
...
void* os_InitStk(Task pfTask, void* pInStkTop, void* pParam)
{
	...
	*pStk = 0xA2;pStk--;
	*pStk = 0xA1;pStk--;
	*pStk = (uint32)pParam;pStk--;	/* param */
	*pStk = (uint32)pfTask;
	return pStk;
}

/**
 * For co-operative OS, switch should save callee save register only.
*/
__attribute__((naked)) void os_Switch(void)
{
	__asm__ __volatile__("addi sp,sp,-52");

	__asm__ __volatile__("sw ra, 0(sp)");
	__asm__ __volatile__("sw s0, 4(sp)");
	__asm__ __volatile__("sw s1, 8(sp)");
	__asm__ __volatile__("sw s2, 12(sp)");
	__asm__ __volatile__("sw s3, 16(sp)");
	__asm__ __volatile__("sw s4, 20(sp)");
	__asm__ __volatile__("sw s5, 24(sp)");
	__asm__ __volatile__("sw s6, 28(sp)");
	__asm__ __volatile__("sw s7, 32(sp)");
	__asm__ __volatile__("sw s8, 36(sp)");
	__asm__ __volatile__("sw s9, 40(sp)");
	__asm__ __volatile__("sw s10, 44(sp)");
	__asm__ __volatile__("sw s11, 48(sp)");

#if 1
	__asm__ __volatile__("lw a0, %[tcb]"::[tcb]"m"(pCurTCB));
	__asm__ __volatile__("sw sp, 0(a0)");

	os_SetNextTask();

	__asm__ __volatile__("lw a0, %[tcb]"::[tcb]"m"(pCurTCB));
	__asm__ __volatile__("lw sp, 0(a0)");
#else
	__asm__ __volatile__("sw sp, %[tcb]":[tcb]"=m"(pCurTCB):);
	os_SetNextTask();
	__asm__ __volatile__("lw sp, %[tcb]"::[tcb]"m"(pCurTCB));
#endif

	__asm__ __volatile__("lw ra, 0(sp)");
	__asm__ __volatile__("lw s0, 4(sp)");
	__asm__ __volatile__("lw s1, 8(sp)");
	__asm__ __volatile__("lw s2, 12(sp)");
	__asm__ __volatile__("lw s3, 16(sp)");
	__asm__ __volatile__("lw s4, 20(sp)");
	__asm__ __volatile__("lw s5, 24(sp)");
	__asm__ __volatile__("lw s6, 28(sp)");
	__asm__ __volatile__("lw s7, 32(sp)");
	__asm__ __volatile__("lw s8, 36(sp)");
	__asm__ __volatile__("lw s9, 40(sp)");
	__asm__ __volatile__("lw s10, 44(sp)");
	__asm__ __volatile__("lw s11, 48(sp)");
